...
def start_shutdown():
    ...
    computer = value1.get()
    if len(computer) >= 3:
        res = subprocess.Popen("shutdown -s -f -t 1 -m " + computer, stdout=subprocess.PIPE, shell=True)

        if res.returncode == 5 or res.returncode == 53:
            showwarning(__filename__, "Erreur : Accès refusé (permissions insuffisantes) !")
        shutdownnet.destroy()
    else:
